...
def part1(input):
    ...
    for line in input:
        ...
        ok = True
        number = line.split(": ")
        # Per avere il numero del game
        #print(number[0][-1])

        games = number[1].split(";")
        print(games)
        ...
        for game in games:
            ...
            for val in values:
                ...
                if gem[1] == "red":
                    ...
                    green = green + int(gem[0])
                
            
            if red > totRed or blue > totBlue or green > totGreen:
                ok = False
                break
        
        if ok == True:
            tot += int((number[0].split())[1])
...
def part2(input):
    ...
    for line in input:
        ...
        maxGreen = 0
        maxBlue = 0
        number = line.split(": ") # ["Game n", "xxx , xxx , xxx ; xxx , xxx"]
        # Per avere il numero del game
        #print(number[0][-1])
        ...
        for game in games:
            ...
            for val in values:
                gem = val.split()
                if gem[1] == "red":
                    maxRed = max( int(gem[0]), maxRed)
                elif gem[1] == "blue":
                    maxBlue = max(int(gem[0]), maxBlue )
                else:
                    maxGreen = max(int(gem[0]), maxGreen)
                
        tot = tot + (maxRed * maxBlue * maxGreen)
